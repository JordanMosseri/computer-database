...
public class Mapper {
	...
	public static Computer mapper(ResultSet rs) throws SQLException {
		...
		Computer c =  new Computer(rs.getInt("id"), rs.getString("name"), new Date(introduced), new Date(removed), new Company(rs.getString("company.name")));//company.name company_id
		return c;
	}
}
